...
def tnm_selection(
    n, adj_mat, sol, max_tnm, mut_md, tb_size, tb_list, fq_dict, best_cost
):
    """
    ...
    for _ in range(max_tnm):
        ...
        v_1, v_2 = (
            (sol[i], sol[j]) if sol[i] < sol[j] else (sol[j], sol[i])
        )  # v_1 < v_2 make indexing in tb_list and fq_dict convenient
        ...
        if (v_1, v_2) not in tb_list:  # if not tabu
            ...
            if delta < best_delta_1:
                ...
                best_j_1 = j
    if (
        best_delta_1 < best_delta_0 and cost + best_delta_1 < best_cost
    ):  # break the tabu
        v_1, v_2 = (
            (sol[best_i_1], sol[best_j_1])
            if sol[best_i_1] < sol[best_j_1]
            else (sol[best_j_1], sol[best_i_1])
        )
...
def tb(n, adj_mat, tb_size, max_tnm, mut_md, term_count):
    ...
    count = 0
    while True:
        sol, cost, tb_list, fq_dict = tnm_selection(
            n, adj_mat, sol, max_tnm, mut_md, tb_size, tb_list, fq_dict, best_cost
        )
